...
def default_codelab_instructions() -> str:
    ...
    return (
        ...
        "- A curated n8n skill library lives in your harness workspace under `skills/`. Start at "
        "`skills/README.md` (the router), then `workspace_read` the SKILL.md of the skill that matches "
        "the task: workflow structure (n8n-workflow-patterns), node setup (n8n-node-configuration), "
        "expressions (n8n-expression-syntax), Code nodes (n8n-code-javascript / n8n-code-python), error "
        "handling (n8n-error-handling), validation (n8n-validation-expert), AI agents (n8n-agents). Use "
        "`workspace_search` to find guidance by keyword. Follow the skill rather than relying on recall.\n"
        "- If n8n-mcp tools are available (search_nodes, get_node, validate_workflow, ...), use them to "
        "confirm real node types + parameters and to VALIDATE a workflow before you return it.\n\n"
        "General:\n"
        ...
        "- For anything non-trivial, consult n8n-code-javascript (or n8n-code-python) in the skill library first.\n\n"
        ...
        "- Consult n8n-workflow-patterns for structure, then validate with the n8n-mcp tools when available "
        "so node types and parameters are correct.\n"
    )
