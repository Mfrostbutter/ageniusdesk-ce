...
- **BUG-014 fully closed 2026-08-18.** Tolerant Code-node read + `agd-handler-version` marker + stale-handler refresh landed with the phase; the live Error Trigger capture on n8n-dev then confirmed the documented contract exactly: `workflow` is a top-level sibling of `execution`, `execution.workflow` does not exist (the old read yielded "Unknown Workflow" for every error), and `execution.error` carries no `name` field (the "Error" fallback is the contract). Sanitized capture: `tests/fixtures/error_trigger_payload.json`, asserted by `tests/test_error_handler_contract.py`. Operationally significant side-finding: on n8n 2.x the error workflow only fires when ACTIVE (published version) — an inactive handler silently drops every error. `install_handler` already activates by default; the workflow's setup note now says to keep it active.
