...
A toggle at the top left switches the whole view between two modes.
...
Switching modes swaps the editor language and, if the current content does not match the new mode, loads a starter (a code template in Code Node mode, a one-node manual-trigger scaffold in Workflow Builder mode). Content that already matches (JSON you are editing, or code you are editing) is preserved. The template and language pickers are hidden in Workflow Builder mode.
