...
- [x] **Fleet Health contribution API**: a module declares `contributes.fleet_health` (a subpath) and publishes `{label, status, metrics}` rows; the host collects them through one authenticated in-process call (same path in every isolation mode) and merges them into the Fleet Health pane, so module health (cluster nodes, queue workers, NAS disks, tunnel status) renders next to the n8n fleet. **Shipped in v0.6.0** — per-module timeout and degrade-not-fatal; Proxmox is the first contributor. Both host investments are now complete.
