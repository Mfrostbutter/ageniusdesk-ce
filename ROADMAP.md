...
- [x] **Public run-start API + MCP tools for fleet agents**: start any registered fleet agent by id over the versioned public API (API-key auth, trigger scope, shared single-flight semantics with the dashboard), and let vault agents declare `mcp:{server}:{tool}` names that resolve to discovered MCP-server tools (bearer via Secrets-store refs). Together with the ticket sink below, this is the agentic-MSP integration seam. Shipped, unreleased — see CHANGELOG.
