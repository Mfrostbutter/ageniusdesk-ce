...
### Added
- **Agent Fleet support (opt-in LangGraph extra).** A new `langgraph` optional dependency group plus an `AGD_EXTRAS` Docker build arg let an operator bake the LangGraph stack (`langgraph`, `langchain-core`, `langchain-anthropic`, `langchain-openai`, `langsmith`) into the image. The default image stays lean (`assistant` only); build with `--build-arg AGD_EXTRAS="assistant,langgraph"` (or `pip install '.[langgraph]'`) to run the new **Agent Fleet** community module, which manages a fleet of LangGraph agents (ops-triage ReAct, fix-proposer human-in-the-loop, health-reporter parallel fan-out) with a live polled graph view, approve/resume, and LangSmith tracing. The module ships in the community-modules repo and runs in_process; see `docs/specs/2026-06-28-agent-fleet-langgraph-spec.md`.

### Fixed
- **Workflow import no longer 400s on a full n8n export.** n8n's public create API is strict (`additionalProperties: false`), so importing a complete export failed with `request/body must NOT have additional properties` whenever it carried a field the importer's denylist missed (e.g. `isArchived` on newer n8n). The importer now builds the create payload from an allowlist (`name`, `nodes`, `connections`, `settings`), so any extra or future export field is dropped and the import succeeds.
